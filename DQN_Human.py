# -*- coding: utf-8 -*-
"""
Created on Thu May  4 14:13:54 2017
...
def getModel(mode):
	...
	adam = Adam(lr=0.0025)
	if mode == 'Run':
		model.load_weights("weights-human.h5")
...
def trainModel(predict_model,actual_model,mode):
	...
	while True:
		...
		if t < 3000 and mode == 'Train':
			 flap = handle_player_events()
		elif random.random() < ep and mode == 'Train':
		...
		if t > 3000 and mode == 'Train':
			...
			for i in range(len(batch)):
				...
				if batch[i][4] == True:
					...
					q = max(actual_model.predict(batch[i][1])[0])
		...
		if t%1000 == 0 and mode == 'Train':
			...
			predict_model.save_weights("weights-human.h5", overwrite=True)
